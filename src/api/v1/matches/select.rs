...
pub async fn put(
  ...
  match config.current_state{
    AppState::EDITING => {
      ...
      app.config().update(doc! { "current_match": id.clone() }).await;
      let _ = app.live().tx.lock().await.send(Message::Text(json!({ "type": "select-match", "match": { "_id": id }, "from": res.unwrap()._id }).to_string()));
